...
# Convert any cell with a '?' value to a NA value. Then can use is.na() or na.omit() commands.
adult[adult == '?'] <- NA

missmap(adult, y.at=c(1),  y.labels = c(''),  col=c('yellow','black'))
# use  y.at=c(1),  y.labels = c('') together to drop the y-labels.
...
ggplot(adult, aes(region)) + geom_bar(aes(fill=income), color='black') + theme_bw() +
       theme(axis.text.x = element_text(angle = 90, hjust = 1)) # change x-label text direction
...
test$predicted.income <- predict(step.model, newdata=test, type="response")
